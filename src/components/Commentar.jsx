...
const Comment = memo(({ comment, formatDate, index, isPinned = false, onPin, currentUserName }) => (
  <div 
      ...
      <div className="flex-grow min-w-0">
        <div className="flex items-center justify-between gap-4 mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {comment.user_name === currentUserName && !isPinned && (
              <button
                onClick={() => onPin(comment.id)}
                className="text-xs text-indigo-400 hover:text-indigo-300 transition ml-2"
              >
                📌 Pin
              </button>
            )}
...
useEffect(() => {
  ...
  try {
    ...
    const fetchComments = async () => {
        ...
        const pinned = data.find(comment => comment.is_pinned === true);
        const others = data.filter(comment => comment.is_pinned !== true);
    ...
    // Real-time subscription: listen for *any* change on the table
    const subscription = supabase
        ...
        .on(
            ...
            () => {
                fetchComments(); // Refresh comments on insert/update/delete
            }
        )
        .subscribe();
...
useEffect(() => {
   ...
   const handleCommentSubmit = useCallback(async ({ newComment, userName, imageFile }) => {
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            .select()
            .single(); // ✅ get the inserted comment back
                    ...
                    {pinnedComment && (
                        <div data-aos="fade-down" data-aos-duration="800">
                            <Comment 
                                ...
                                index={0}
                                isPinned={true}
                                onPin={handlePinComment}           // ✅ Pass handler
                                currentUserName="manisankar"  
                            />
                        </div>
                    )}
  ...
  <Comment 
    ...
    isPinned={comment.id === pinnedComment?.id}
    onPin={handlePinComment} // 👈 Add this
    currentUserName="manisankar" // 👈 Replace with your admin username if needed
  />
))
