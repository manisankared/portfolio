...
import { Code, Award, Medal, Boxes } from "lucide-react";
...
export default function FullWidthTabs() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [projectsResponse, certificatesResponse, badgesResponse] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const cachedBadges = localStorage.getItem("badges");

    if (cachedProjects && cachedCertificates && cachedBadges){
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 md:gap-5 gap-4">
      {displayedBadges.map((badge, index) => (
        <div
          ...
          data-aos-duration={index % 3 === 0 ? "1000" : index % 3 === 1 ? "1200" : "1000"}
        >
          <Certificate ImgSertif={badge.img} />
        </div>
      ))}
    </div>
  </div>
